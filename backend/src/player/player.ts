...
export class Player extends DomainBase<PlayerDTO> {
	...
	async toDTO(): Promise<PlayerDTO> {
		return {
			...
			quads: await Promise.all(this.quads.map(async (q) => q.toDTO())) as QuadsDTO,
			score: this.score,
			status: this.status.toString(),
			trips1: await Promise.all(this.trips1.map(async (t) => t.toDTO())) as TripsDTO,
			trips2: await Promise.all(this.trips2.map(async (t) => t.toDTO())) as TripsDTO,
